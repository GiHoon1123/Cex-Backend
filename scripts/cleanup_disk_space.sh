...
#   - 크론에서 매일 실행됩니다 (한국시간 새벽 2시 = UTC 17시)
#
# 주요 기능:
#   1. 디스크 사용량 확인 (90% 이상 시 강제 데이터 삭제)
#   2. Docker 사용하지 않는 이미지 삭제
#   3. Docker 빌드 캐시 정리 (7일 이상 된 캐시)
#   4. PostgreSQL VACUUM ANALYZE 실행
#   5. PostgreSQL WAL 상태 확인
#   6. 시스템 패키지 캐시 정리 (dnf/yum)
#
# 강제 데이터 삭제 (디스크 90% 이상):
#   - PostgreSQL에 직접 연결하여 모든 테이블 데이터 삭제
#   - 외래키 제약조건 일시 비활성화 후 삭제
#   - VACUUM FULL 실행하여 공간 회수
#   - 여전히 90% 이상이면 컨테이너 재시작 시도
#
# 로그:
#   - /home/ec2-user/logs/disk_cleanup.log
#
# 실행:
#   - 크론에서 자동 실행: setup_disk_cleanup_cron.sh로 설정
#   - 수동 실행: /usr/local/bin/cleanup_disk_space.sh
# =====================================================
