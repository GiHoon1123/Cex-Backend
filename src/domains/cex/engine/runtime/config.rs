...
impl CoreConfig {
    ...
    pub fn from_env() -> Self {
        ...
        match env.as_str() {
            ...
            "prod" => {
                // 프로덕션 환경 (2코어) - 코어 고정 비활성화 (OS/네트워크 스택과 충돌 방지)
                // 우선순위 99만 사용하여 엔진 스레드가 우선 실행되도록 함
                // 2코어에서는 코어 고정 시 OS 스레드(네트워크 등)가 블록될 수 있음
                // engine_core, wal_core는 타입상 usize이지만 실제로는 set_core(None) 호출
                Self {
                    engine_core: 999,  // 특수 값 (코어 고정 비활성화를 위해 사용, 실제 코어 번호 아님)
                    wal_core: 999,     // 특수 값 (코어 고정 비활성화를 위해 사용, 실제 코어 번호 아님)
